...
public class WordLadder {
	...
	public static void main(String[] args) {
		...
		String start = "sand";
		String end = "acne";
		String[] arr = { "slit", "bunk", "wars", "ping", "viva", "wynn",
			...
			for (int i = 0; i < size; i++) {
				...
				for (char c = 'a'; c <= 'z'; c++) {
					for (int j = 0; j < strlen; j++) {
						if (c == arr[j])
							continue;
						String tmp = replace(arr, c, j);
						if (tmp.equals(end))
							return len + 1;
						if (dict.contains(tmp)) {
							// int diff1 = diff(current, end);
							// int diff2 = diff(tmp, end);
							// if (diff1 >= diff2) {
							queue.add(tmp);
							dict.remove(tmp);
							// }
	...
	public String replace(char[] array, char c, int idx) {
		char tmp = array[idx];
		array[idx] = c;
